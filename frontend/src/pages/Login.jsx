...
const Login = ({ setToken, setRole }) => {
  ...
  const handleOtpVerification = async () => {
    ...
    try {
      const response = await axios.post(`${backendURL}/api/${accountType}/verify-otp`, {
        email,
        otp,
      });
    ...
    <section className="bg-base-200 min-h-screen flex items-center justify-center px-4">
      <div className="card w-full max-w-md bg-base-100 shadow-xl p-6 space-y-5">
        ...
        <div className="flex border-b border-base-300">
          <button
            type="button"
            onClick={() => { setIsSignUp(false); setOtpSent(false); setError(''); }}
          ...
          <button
            type="button"
            onClick={() => { setIsSignUp(true); setOtpSent(false); setError(''); }}
            className={`btn btn-ghost flex-1 rounded-none ${isSignUp ? 'border-b-2 border-primary text-primary' : 'text-base-content/60'}`}
          >
            Sign Up
          </button>
        ...
        {/* Form */}
        <form onSubmit={isSignUp ? (otpSent ? handleOtpVerification : (e) => { e.preventDefault(); handleOtpRequest(); }) : handleLogin} className="space-y-4">
          {/* Account Type */}
          <div>
          ...
          <button type="submit" className="btn btn-primary w-full">
            {isSignUp ? (otpSent ? 'Verify OTP' : 'Send OTP') : 'Login'}
          </button>
